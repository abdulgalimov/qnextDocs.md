...
— если нужно загрузить медиа из чужого сообщения**: ```${update.message.reply```to```message}```
...
По умолчанию, реакция берет самый большой из доступных размеров и его загружает. Если вам нужен меньший размер, то необходимо создать объект с этими данными в формате file```id и type, пример ниже:

Собираем объект:

1) localVarSet file.fileId = ```${путь до file```id}``` **Галочка Собрать объект**
