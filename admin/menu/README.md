...
- Если отключаете превью в подразделах — это касается только этого подраздела.
...
${buttonsVar.v2} - ${targetUser.id}

${buttonsVar.v3} - ${chat.id}

${buttonsVar.v4} - ${profile.name.value}
```
...
Управление видимостью кнопок навигации по меню в текущем разделе меню.
...
Позволяет ограничить доступ к разделу меню по любому заданному вами условию. Например по наличию юзернейма у пользователя, с помощью ограничения "hasUsername✅" 

Это локальное ограничение, которое будет распространяться только на этот раздел (подраздел).
