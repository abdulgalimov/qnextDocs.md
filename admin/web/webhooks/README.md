...
![](./4.png)
 ...
 В тригере вебхука есть параметр ```${request...}```, который содержит в себе тип запроса (GET/POST), переданные параметры (query, body) и заголовки (headers). В тригере также можно ответить на запрос, с помощью реакции ⚡️[webhookResponse](/docs/admin/web/webhookresponse)
