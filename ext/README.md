...
**Примеры переменных:**
* _${user.name}_ - отобразит имя пользователя
* _${user.id}_ - отобразит ID пользователя
* _$[update.message.text}_ - отобразит введенный текст
* _${update.message.message_id}_ - отобразит ID отправленного сообщения
...
::: tip ❕️
⚠️Полезно: Переменные поддерживают мультипуть с помощью запятой `,`. Если указать переменную формата _${user.id,user.name}_ - переменная отобразит первый найденный параметр.
:::
::: tip ❕️
...
* _${user.id|notFound: произвольный текст}_
* _${update.message.text|notFound: произвольный текст}_
...
* _(рекомендация)_ для удобства чтения, рекомендуется разделять параметры новой строкой:
```plain 
параметр: 1;
параметр: 2;
параметр: 3;
