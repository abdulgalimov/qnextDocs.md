...
По умолчанию бот пытается загрузить профиль текущего пользователя, если необходимо загрузить профиль другого пользователя, например `targetUser`, тогда надо воспользоваться параметром `path`:
```plain 
!{profile|
  type: value;
  name: Баллы;
