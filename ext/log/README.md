...
ссылке типа:  ```qnext.app/logs/@BotUsername```.
...
**request** - сформированный ботом json объект, содержащий в себе данные о том, в
ыполнение какого метода выполняется.
...
Форма, единственный и уникальный раздел, который отображает введенные пользователем данные в 2х форматах:
...
* \$ chat.id = -1001000000000 or update.message.chat.id = -1001000000000 - будут отображены логи, в которых есть оба параметра. 
:::
