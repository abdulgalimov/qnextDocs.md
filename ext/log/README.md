...
На скриншоте отображены 3 базовые объекта, являющиеся постоянным/обязательным трататата
...
| Раздел | Описание |
| --- | --- |
| [${update...}](/ext/#update) | Выводит все исходные (сырые, необработанные) данные предоставленные Телеграмом |
| ${localVar...} | Выводит созданные локальные переменные в момент использования реакции log |
| ${dynamicalVar...} | Выводит созданные переменные в момент использования реакции log |
| ${requestType...} | Выводит отправителя апдейта: telegram, webhook, trello, pay, newsletter, dashboard |
| ${bot...} | Выводит параметры бота |
| [${user...}](/ext/log/#user) | Выводит доступные боту сведения об инициаторе действия |
| ${client...} | Выводит параметры клиента пользователя. Все сведения в этом разделе, отдает Телеграм  |
| ${trigger...} | Выводит параметры запущенного триггера |
| ${reaction...} | Выводит информацию о результатах выполнения реакции |
...
* В логах работает поиск по полному совпадению, а так же по сущностями: 
